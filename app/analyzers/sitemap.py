...
def analyze_sitemap(page: FetchResult) -> CategoryResult:
    ...
    try:
        ...
        issues.append(Issue(severity="error", message=f"Could not fetch sitemap.xml: {str(e)[:100]}",
                            impact="high", recommendation="Ensure sitemap.xml is accessible at your domain root.",
                            evidence=sitemap_url))
        metrics = {"accessible": False, "url_count": 0, "stale_entries": 0, "current_url_in_sitemap": False}
        return CategoryResult(name="sitemap", score=max(0, score - 40), issues=issues, metrics=metrics)

    if resp.status_code != 200:
        issues.append(Issue(severity="error", message=f"sitemap.xml not found (HTTP {resp.status_code})",
                            impact="high", recommendation="Create and submit a sitemap.xml to help search engines discover your pages."))
        metrics = {"accessible": False, "url_count": 0, "stale_entries": 0, "current_url_in_sitemap": False}
        return CategoryResult(name="sitemap", score=max(0, score - 40), issues=issues, metrics=metrics)
    ...
    if "xml" not in resp.headers.get("content-type", "").lower():
        issues.append(Issue(severity="warning", message="sitemap.xml exists but content-type is not XML",
                            impact="medium", recommendation="Set the Content-Type header to application/xml for sitemap.xml."))
        metrics = {"accessible": True, "url_count": 0, "stale_entries": 0, "current_url_in_sitemap": False}
        return CategoryResult(name="sitemap", score=max(0, score - 20), issues=issues, metrics=metrics)
    ...
    if sub_sitemaps:
        ...
        issues.append(Issue(severity="info", message=f"Sitemap contains {url_count} URL entries"))
        ...
        if lastmod_texts:
            ...
            for dt_str in lastmod_texts:
                try:
                    ...
                    pass
            if stale_entries:
                issues.append(Issue(severity="warning", message=f"{stale_entries} URL(s) have <lastmod> older than 1 year",
                                    impact="low", recommendation="Update stale lastmod dates or remove outdated entries."))
                score -= 10
        else:
        ...
        if cross_domain:
            issues.append(Issue(severity="warning", message=f"{cross_domain} URL(s) point to a different domain",
                                impact="low", recommendation="Remove cross-domain URLs from your sitemap."))
            score -= 10
    else:
        issues.append(Issue(severity="warning", message="Sitemap exists but contains no URL entries",
                            impact="medium", recommendation="Add URL entries to your sitemap.xml."))
        score -= 15
        all_locs = []

    # Check if current URL is in sitemap
    normalized = [l.rstrip("/") for l in all_locs]
    current_url_in_sitemap = page.url in all_locs or page.url.rstrip("/") in normalized
    if current_url_in_sitemap:
        ...
        issues.append(Issue(severity="info", message="Current URL not found in sitemap",
                            impact="low", recommendation="Ensure the audited URL is included in your sitemap."))

    metrics = {
        "accessible": accessible,
        "url_count": url_count,
        "stale_entries": stale_entries,
        "current_url_in_sitemap": current_url_in_sitemap,
    }

    return CategoryResult(name="sitemap", score=max(0, score), issues=issues, metrics=metrics)
